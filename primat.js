var Primats = (function () {
  ...
  var drawImages = function (imgArr) {
    ...
    imgArr.forEach(function (image, i) {
      ...
      xImg = x(-image.x);
      yImg =
        parseFloat(d3.select("rect#id" + image.id).attr("y")) +
        parseFloat(image.y_offset);
  ...
  var drawHierarchy = function (data) {
    ...
    myOrders.forEach(function (order) {
      ...
      subOrders.forEach(function (subOrder, o) {
        ...
        if (subOrder)
          drawRect(myleaves, subOrder, "подотряд.", "subOrder", o * 10 + 6);
        ...
        hypOrders.forEach(function (hypOrder, n) {
          ...
          infraOrders.forEach(function (infraOrder, m) {
              ...
              superFamilys.forEach(function (superFamily, k) {
                ...
                if (superFamily)
                  drawRect(
                    ...
                    "надсем.",
                    "superfamily",
                    k * 100000 + l * 10000 + m * 1000 + n * 100 + o * 10 + 2,
                  );
            ...
            if (infraOrder)
              resizeRect(infraOrder, m * 1000 + n * 100 + o * 10 + 4);
          });
  ...
  var findPrimers = function (data) {
    data.forEach(function (item) {
      if (item.origin) {
        ...
        if (+item.period_start > +parent.period_start) {
          /*if we found the case of primer*/
    ...
    data.forEach(function (d) {
      if (d.origin) {
        ...
        if (brickFrom.node() && brickTo.node()) {
          ...
          var x2From =
            parseFloat(brickFrom.attr("x")) +
            parseFloat(brickFrom.attr("width")) -
            0.5;
          ...
          var x2To =
            parseFloat(brickTo.attr("x")) + parseFloat(brickTo.attr("width"));
  ...
  var drawAnSVG = function () {
    ...
    svg = d3.select("svg");
    svg.attr("width", plotWidth).attr("viewBox", "0 0 " + plotWidth + " 8100");
    g_axis = d3
      .select("#axis")
      .append("svg")
      ...
      layersData.forEach(function (d, i, a) {
        if (i < a.length - 1) {
          if (a[i].date_start > time && a[i + 1].date_start < time) {
            trackPeriodText
              .text(a[i].sublayer + "  " + a[i].layer_ru)
              .attr("x", function () {
                return mousex[0];
              })
              .attr("y", -5);
  ...
  var resizeRect = function (name, i) {
    name = name.replace(/[|(|)|]| |=|\?|\./g, "");
    ypos =
      g.selectAll("g").last().node().getBBox().y +
      g.selectAll("g").last().node().getBBox().height +
      10;
    ...
    var text = g
      .selectAll("g#" + name + i)
      .last()
      .select("text:not(.genus)");
    ///rect.clone(true).attr("stroke","red");
    var maxRightEdge = 0;
    var flaf = false;
    g.selectAll("#" + name + i + " ~ g > rect.rec_fam")
      .nodes()
    ...
    if (rect.node()) {
      hasSubFum = g
        .selectAll("g#" + name + i + ".family")
        .last()
        .select("g  rect.rec_subfam");
      ...
      if (text.node())
        if (newex + text.node().getBBox().width > x(0)) {
          //move text lefter if its on the right edge
    ...
    rect.attr("height", newheight).attr("width", newwidth).attr("x", newex);
  ...
  var drawRect = function (ids, name, prefix, taxon, i) {
    ...
    var emptySubFam = false,
      atLeastOneSubTaxon = false,
      atLeastOneGen = false;
  ...
  var drawBricks = function (bricks, fam) {
    ...
    if (g1.selectAll("g:last-of-type").node())
      ypos =
        g1.select("g:last-of-type").node().getBBox().y +
        g1.select("g:last-of-type").node().getBBox().height;
    ...
    var emptyGen = false,
      ...
      atLeastOneGen = false,
      greenRec = false;
    var firstLocation = bricks[0].location,
      sameGens = false,
    ...
    while (i >= 0) {
      ...
      if (
        i > 0 &&
        bricks[i - 1].period_start == bricks[i].period_start &&
        bricks[i - 1].period_end == bricks[i].period_end &&
        bricks[i - 1].genus == bricks[i].genus
      ) {
    ...
    if (sameGens == true) {
      console.log(
        bricks[0].id + " " + nameOfUnionBrick + " locations=",
        locations,
      );
  ...
  var drawUsualBricks = function (
    ...
    g2.selectAll("rect.brick")
      ...
      .attr("x", (d) => {
        ...
        if (d.period_start == 0 && d.period_end == 0)
          return x(-1 * d.period_start) - 5;
    ...
    g2.selectAll("text.genus")
      .data(bricks)
      .enter()
      .append("text") //create signs for lines
      ...
      .text(function (d) {
        ...
        if (d.genus == "—" || d.genus == "") {
          ...
          if (d.subfamily == "—" || d.subfamily == "") {
            ...
            else fullText = /*d.id + " " +*/ "сем. " + d.family + loc;
          } else {
            if (atLeastOneSubTaxon == true && greenRec)
              fullText = /*d.id +*/ loc; //завершающая пустышка подсемейства
            else fullText = /*d.id + " " +*/ "подсем. " + d.subfamily + loc;
          }
        } else fullText = /*d.id + " " +*/ d.genus + loc;
      ...
      .text(function () {
        if (this.innerHTML.length > desireLength) dots = "...";
        else dots = "";
  ...
  var initFilter = function () {
    ...
    feMerge.append("feMereNode").attr("in", "feFlood");
